		...
		for (uint h = 0; h < CPU_BATCH_SIZE; ++h) {
			...
			if (hash_results[0]) {
				...
				if (finalhash[30] != 0 || finalhash[31] != 0)
					...
					cpu_shares_hwvalid++;
				if (CPU_Hash_Below_Target
				    (finalhash, &tempwork.target_share[0]))
					CPU_Got_share(state, tempdata,
			...
			if (hash_results[1]) {
				...
				if (CPU_Hash_Below_Target
				    (finalhash, &tempwork.target_share[0]))
					CPU_Got_share(state, tempdata + 512,
						      tempwork.target_share,
						      current_server_id);
			}
			if (hash_results[2]) {
				...
				if (CPU_Hash_Below_Target
				    (finalhash, &tempwork.target_share[0]))
					CPU_Got_share(state, tempdata + 1024,
						      tempwork.target_share,
						      current_server_id);
			}
			++*(uint *) & tempdata[108];
