...
const uint P[64] = {
	...
	0xa4f08d87, 0x9e3e8435, 0x2c6dae30, 0x11921afc, 0xb76d720e, 0x245f3661,
	    0xc3a65ecb, 0x43b9e908
};
...
vector < uchar > CalculateMidstate(vector < uchar > workdata)
{
	...
	for (uint i = 0; i < 64; ++i) {
		if (i >= 16)
			work[i] =
			    work[i - 7] + work[i - 16] + Wr(work[i - 2], 17, 19,
							    10) + Wr(work[i -
									  15],
								     7, 18, 3);
		sharound(hash[(0 - i) & 7], hash[(1 - i) & 7],
			 hash[(2 - i) & 7], hash[(3 - i) & 7],
			 hash[(4 - i) & 7], hash[(5 - i) & 7],
			 hash[(6 - i) & 7], hash[(7 - i) & 7], work[i], K[i]);
	}
	for (uint i = 0; i < 8; ++i)
		hash[i] += initial[i];
	vector < uchar > ret;
