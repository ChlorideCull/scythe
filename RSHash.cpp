...
{
	uint n = (((x>>38UL) + ((x>>6)&0xFFFFFFFF)) >> 32) + ((x>>38UL) + ((x>>6)&0xFFFFFFFF)&0xFFFFFFFF);
	return ((n%5)<<6)+(x&63U);
}
//96
uint remu160(ullint x) 
{
	uint n = (((x>>37UL) + ((x>>5)&0xFFFFFFFF)) >> 32) + ((x>>37UL) + ((x>>5)&0xFFFFFFFF)&0xFFFFFFFF);
	return ((n%5)<<5)+(x&31U);
}
//208
...
#define sCount counts[2]
	...
	int x,y,z;
    for(x=1,y=1,z=1;x<nExtra && y<oExtra && z<pExtra;x++,y++,z++)
    {
        qCount+= READ_PAD32( qCount );
        rCount+= READ_PAD32( rCount );
        sCount+= READ_PAD32( sCount );

        if(qCount&0x87878700)        ++work3[remu320(qCount)];
        if(rCount&0x87878700)        ++xork3[remu320(rCount)];
        if(sCount&0x87878700)        ++york3[remu320(sCount)];
        ...
        if(rCount&0x80000000)   { rCount+= READ_PAD8( rCount&0x8080FFFF ); }
        else                    { rCount+= READ_PAD32( rCount&0x7F60FAFB ); }
        if(sCount&0x80000000)   { sCount+= READ_PAD8( sCount&0x8080FFFF ); }
        else                    { sCount+= READ_PAD32( sCount&0x7F60FAFB ); }
		...
		if(qCount&0xF0000000)        ++work3[remu320(qCount)];
		if(rCount&0xF0000000)        ++xork3[remu320(rCount)];
		if(sCount&0xF0000000)        ++york3[remu320(sCount)];
		...
		*((uint32*)&work3[remu316(qCount)]) ^= (qCount>>24)&0xFFFFFFFF;
        *((uint32*)&xork3[remu316(rCount)]) ^= (rCount>>24)&0xFFFFFFFF;
        *((uint32*)&york3[remu316(sCount)]) ^= (sCount>>24)&0xFFFFFFFF;
        
		if((qCount&0x07)==0x03) x++;
        if((rCount&0x07)==0x03) y++;
        if((sCount&0x07)==0x03) z++;
		...
		if((qCount&0x07)==0x01) x++;
        if((rCount&0x07)==0x01) y++;
        if((sCount&0x07)==0x01) z++;
	}
	while(x<nExtra)
	{
        qCount+= READ_PAD32( qCount );
	...
	Sha256(work1, final_hash);
	results[0]=(final_hash[30] == 0 && final_hash[31] == 0 && final_hash[29] < 0x80);
	Sha256(xork1, final_hash);
	results[1]=(final_hash[30] == 0 && final_hash[31] == 0 && final_hash[29] < 0x80);
	Sha256(york1, final_hash);
	results[2]=(final_hash[30] == 0 && final_hash[31] == 0 && final_hash[29] < 0x80);
}
