...
void SubmitShare(Curl & curl, Share & w, unsigned char *scratchbuf)
{
	if (w.data.size() % 128 != 0) {
		cout << "SubmitShare: Size of share is " << w.data.
		    size() << ", should be a multiple of 128" << endl;
		return;
	}
	try {
		if (globalconfs.coin.protocol == "bitcoin") {
			...
			SHARETEST_VALUE sharevalid =
			    ShareTest_BTC((uint *) & w.data[0],
					  (uint *) & vec[0]);
			if (sharevalid == ST_HNOTZERO
			    && OpenCL::GetVectorSize() >= 2) {
				w.data[79] ^= 0x80;
				sharevalid =
				    ShareTest_BTC((uint *) (&w.data[0]),
						  (uint *) & vec[0]);
				if (sharevalid == ST_HNOTZERO
				    && OpenCL::GetVectorSize() >= 4) {
					w.data[79] ^= 0x40;
					sharevalid =
					    ShareTest_BTC((uint *) (&w.data[0]),
							  (uint *) & vec[0]);
			...
			SHARETEST_VALUE sharevalid =
			    scanhash_scrypt(&w.data[0], scratchbuf,
					    &w.target[0]);
			if (sharevalid == ST_HNOTZERO)
				shares_hwinvalid++;
...
void *LongPollThread(void *param)
{
	...
	cout << "Long polling URL: [" << LP_url << "]. trying to parse." <<
	    endl;
	clock_t lastcall = 0;
 ...
 couldnt_parse:
	cout << "Couldn't parse long polling URL [" << LP_url <<
	    "]. turning LP off." << endl;
	pthread_exit(NULL);
	return NULL;
}
...
void App::SetupCurrency()
{
	map < string, Coin > coins;
	{
		...
		{
			if (c.config.GetValue < string > ("aggression") ==
			    "max") {
				c.global_worksize = 1 << 11;
				c.max_aggression = true;
			} else {
				c.global_worksize =
				    (1 << c.config.GetValue < uint >
				     ("aggression"));
		...
		c.threads_per_gpu =
		    c.config.GetValue < uint > ("threads_per_gpu");
		c.cputhreads =
		    c.config.GetValue < uint > ("cpu_mining_threads");
		c.cpu_algorithm =
		    c.config.GetValue < string > ("cpu_algorithm");
	...
	if (coins.find(minedcoin) == coins.end()) {
		cout << "Coin chosen for mining \"" << minedcoin <<
		    "\" not found." << endl;
		throw string("");
	} else
	...
	if (globalconfs.coin.host == "" ||
	    ...
	    globalconfs.coin.user == "" || globalconfs.coin.pass == "")
		throw string("Config ") + globalconfs.coin.name +
	    ".conf is missing one of host/port/user/pass.";
	...
	if (globalconfs.coin.cputhreads == 0
	    && globalconfs.coin.threads_per_gpu == 0) {
		throw
		    string
		    ("No CPU or GPU mining threads.. please set either cpu_mining_threads or threads_per_gpu to something other than 0.");
	}
}
...
void App::LoadServers()
{
	...
	for (uint i = 0; i < servercount; ++i) {
		ServerSettings s;
		s.host =
		    globalconfs.coin.config.GetValue < string > ("host", i);
		s.port =
		    globalconfs.coin.config.GetValue < unsigned short >("port",
									i);
		s.user =
		    globalconfs.coin.config.GetValue < string > ("user", i);
		s.pass =
		    globalconfs.coin.config.GetValue < string > ("pass", i);
		s.proxy =
		    globalconfs.coin.config.GetValue < string > ("proxy", i);
		servers.push_back(s);
	}
	current_server_id = 0;
}
...
void App::Main(vector < string > args)
{
	...
	if (args.size() > 2) {
		cout << "Please use config files to set host/port/user/pass" <<
		    endl;
		return;
	}
	if (args.size() == 2)
	...
	for (uint i = 0; i < numdevices; ++i)
		globalconfs.devices.push_back(config.GetValue < uint >
					      ("device", i));
	...
	if (globalconfs.coin.cputhreads == 0) {
		throw
		    string
		    ("cpu_mining_threads is zero. Nothing to do, quitting.");
	}
#endif
	...
	if (globalconfs.coin.config.GetValue < uint > ("sharethreads")) {
		for (uint i = 1;
		     i < globalconfs.coin.config.GetValue < uint >
		     ("sharethreads"); ++i)
			pthread_create(&sharethread, NULL, ShareThread, &curl);
	}
	...
	int work_update_period_ms =
	    globalconfs.coin.config.GetValue < uint > ("getwork_rate");
	...
	if (longpoll_active) {
		...
		lp_params.app = this;
		lp_params.curl = &curl;
		pthread_create(&longpollthread, NULL, LongPollThread,
			       &lp_params);
	}
	...
	while (!shutdown_now) {
		...
		if (timeclock - current_work_time >=
		    WORK_EXPIRE_TIME_SEC * 1000) {
			if (!current_work.old) {
				cout << humantime() <<
				    "Work too old... waiting for getwork.    "
				    << endl;
			}
			current_work.old = true;
		}
		...
		if (timeclock - sharethread_update_time >=
		    SHARE_THREAD_RESTART_THRESHOLD_SEC * 1000) {
			cout << humantime() <<
			    "Share thread messed up. Starting another one.   "
			    << endl;
			pthread_create(&sharethread, NULL, ShareThread, &curl);
		}
		if (getwork_now
		    || timeclock - workupdate >= work_update_period_ms) {
			uint timmii = ticker();
			current_server_id =
			    (current_server_id + 1) % servers.size();
			Parse(curl.GetWork(servers[current_server_id]));
			timmii = ticker() - timmii;
			if (timmii > 5000)
				cout << "Getwork took " << timmii /
				    1000.0 << " s!  " << endl;
			getwork_now = false;
		}
		if (timeclock - ticks >= 1000) {
			...
			if (shares_valid + shares_invalid != 0)
				stalepercent =
				    100.0f * float (shares_invalid) /
			    float (shares_invalid + shares_valid);
			if (ticks - starttime == 0)
				cout << dec << "   ??? kH/s, shares: " <<
				    shares_valid << "|" << shares_invalid <<
				    ", stale " << stalepercent << "%, " <<
				    (ticks - starttime) / 1000 << "s    \r";
			else {
				stringstream stream;
				stream.precision(4);
				if (totalhashesGPU != 0) {
				...
				if (totalhashesCPU != 0) {
					stream << "CPU " <<
					    double (totalhashesCPU) / (ticks -
								       starttime)
					    << "kH/s, ";
				}
				stream << "shares: " << shares_valid << "|" <<
				    shares_invalid << ", stale " << stalepercent
				    << "%, ";
				float hwpercent =
				    100.0f * float (shares_hwinvalid) /
				    float (shares_hwinvalid + shares_hwvalid);
				if (shares_hwinvalid + shares_hwvalid > 0)
					stream << "GPU errors: " << hwpercent <<
					    "%, ";
				float cpuhwpercent =
				    100.0f * float (cpu_shares_hwinvalid) /
				    float (cpu_shares_hwinvalid +
					   cpu_shares_hwvalid);
				if (cpu_shares_hwinvalid + cpu_shares_hwvalid >
				    0)
					stream << "CPU errors: " << cpuhwpercent
					    << "%, ";
				cout << dec << stream.str() << "~" << 1000.0 *
				    (shares_hwvalid +
				     cpu_shares_hwvalid) / double (ticks -
								   starttime) *
				    globalconfs.coin.
				    sharekhs << " kH/s, " << (ticks -
							      starttime) /
...
void App::Parse(string data)
{
	...
	if (data == "") {
		...
		if (servers.size() > 1)
			cout << "Trying next server in a few seconds... " <<
			    endl;
		else
...
void App::Parse_SLC(string data)
{
	...
	if (result.isObject()) {
		...
		if (!targetprinted) {
			targetprinted = true;
			cout << "target_share: " << result["target_share"].
			    asString() << endl;
		}
		newwork.target_share =
		    HexStringToVector(result["target_share"].asString().
				      substr(2));
		...
		cout << humantime() << "Code " << error["code"].
		    asInt() << ", \"" << error["message"].
		    asString() << "\"" << endl;
	}
 got_error:
...
void App::Parse_BTC(string data)
{
	...
	if (result.isObject()) {
		...
		if (has_midstate)
			newwork.midstate =
			    HexStringToVector(result["midstate"].asString());
		else
			newwork.midstate =
			    CalculateMidstate(HexStringToVector
					      (result["data"].asString().
					       substr(0, 128)));
		newwork.data =
		    HexStringToVector(result["data"].asString().substr(0, 160));
		newwork.old = false;
		newwork.time = ticker();
		current_work_time = ticker();
		...
		{
			vector < uchar > targetuchar =
			    HexStringToVector(result["target"].asString());
