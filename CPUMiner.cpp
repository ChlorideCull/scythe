...
void CPU_Got_share(Reap_CPU_param * state, uchar * tempdata,
		   vector < uchar > &target, uint serverid)
{
...
void CPUMiner::Init()
{
	...
	if (globalconfs.coin.name != "solidcoin") {
		if (globalconfs.coin.cputhreads != 0)
			cout << "Only SolidCoin CPU mining is supported" <<
			    endl;
		return;
	}
	...
	for (map < string, void *(*)(void *)>::iterator it = funcs.begin();
	     it != funcs.end(); ++it) {
		if (it != funcs.begin())
			cout << ", ";
		cout << it->first;
	...
	if (cpualgo == NULL) {
		if (globalconfs.coin.cpu_algorithm != "")
			cout << "CPU algorithm " << globalconfs.coin.
			    cpu_algorithm << " not found." << endl;
		cout << "Using default: vector3" << endl;
		cpualgo = Reap_CPU_V3;
	} else
		cout << "Using CPU algorithm: " << globalconfs.coin.
		    cpu_algorithm << endl;
	...
	cout << "Creating " << CPUstates.size() << " CPU thread" << (CPUstates.
								     size() ==
								     1 ? "" :
								     "s") << "."
	    << endl;
	for (uint i = 0; i < CPUstates.size(); ++i) {
		...
		pthread_create(&CPUstates[i].thread, &attr, cpualgo,
			       (void *)&CPUstates[i]);
