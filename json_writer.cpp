		...
		case objectValue:
			{
				...
				document_ += "{";
				for (Value::Members::iterator it =
				     members.begin(); it != members.end();
				     ++it) {
 ...
 :	    rightMargin_(74)
	...
	bool StyledWriter::isMultineArray(const Value & value) {
		...
		if (!isMultiLine)	// check if line length > max line length
		{
			...
			for (int index = 0; index < size && !isMultiLine;
			     ++index) {
				writeValue(value[index]);
				lineLength +=
				    int (childValues_[index].length());
				isMultiLine = isMultiLine
				    && hasCommentForValue(value[index]);
			}
			addChildValues_ = false;
	...
	void
	 StyledWriter::writeCommentAfterValueOnSameLine(const Value & root) {
		...
		if (root.hasComment(commentAfter)) {
			document_ += "\n";
			document_ +=
			    normalizeEOL(root.getComment(commentAfter));
			document_ += "\n";
		}
	}
 ...
 :	    document_(NULL)
	...
	void
	 StyledStreamWriter::unindent() {
		...
		indentString_.resize(indentString_.size() -
				     indentation_.size());
	}
	...
	void
	 StyledStreamWriter::
	    writeCommentAfterValueOnSameLine(const Value & root) {
		if (root.hasComment(commentAfterOnSameLine))
			*document_ << " " +
			    normalizeEOL(root.
					 getComment(commentAfterOnSameLine));
