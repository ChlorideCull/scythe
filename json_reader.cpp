...
// ////////////////////////////////
	...
	static inline bool
	    in(Reader::Char c, Reader::Char c1, Reader::Char c2,
	...
	static bool
	    containsNewLine(Reader::Location begin, Reader::Location end) {
		for (; begin < end; ++begin)
			if (*begin == '\n' || *begin == '\r')
				return true;
		return false;
	...
	static std::string codePointToUTF8(unsigned int cp) {
		...
		if (cp <= 0x7f) {
			...
			result[1] =
			    0x80 | static_cast < char >((0x3f & (cp >> 6)));
			result[0] =
			    0xE0 | static_cast < char >((0xf & (cp >> 12)));
		} else if (cp <= 0x10FFFF) {
			result.resize(4);
...
// //////////////////////////////////////////////////////////////////
 ...
 :	    features_(features) {
	...
	bool Reader::readValue() {
		...
		if (collectComments_ && !commentsBefore_.empty()) {
			currentValue().setComment(commentsBefore_,
						  commentBefore);
			commentsBefore_ = "";
		}
	...
	bool Reader::expectToken(TokenType type, Token & token,
				 const char *message) {
		readToken(token);
		if (token.type_ != type)
	...
	void
	 Reader::addComment(Location begin,
			    Location end, CommentPlacement placement) {
		assert(collectComments_);
		if (placement == commentAfterOnSameLine) {
			assert(lastValue_ != 0);
			lastValue_->setComment(std::string(begin, end),
					       placement);
		} else {
	...
	bool Reader::decodeNumber(Token & token) {
		bool isDouble = false;
		for (Location inspect = token.start_; inspect != token.end_;
		     ++inspect) {
	...
	bool Reader::decodeUnicodeCodePoint(Token & token,
		...
		if (unicode >= 0xD800 && unicode <= 0xDBFF) {
			...
			if (*(current++) == '\\' && *(current++) == 'u') {
				if (decodeUnicodeEscapeSequence
				    (token, current, end, surrogatePair)) {
					unicode =
					    0x10000 +
					    ((unicode & 0x3FF) << 10) +
					    (surrogatePair & 0x3FF);
